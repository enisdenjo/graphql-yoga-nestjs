...
> extends AbstractGraphQLDriver<YogaDriverConfig<Platform>> {
  ...
  public async start(options: YogaDriverConfig<Platform>) {
    ...
    options = {
      ...
      maskedErrors: options.maskedErrors != null ? options.maskedErrors : false,
      // disable graphiql in production
      graphiql: options.graphiql != null ? options.graphiql : process.env.NODE_ENV !== 'production',
    };
...
> extends AbstractYogaDriver<Platform> {
  ...
  public async start(options: YogaDriverConfig<Platform>) {
    const opts = await this.graphQlFactory.mergeWithSchema<YogaDriverConfig<Platform>>(options);
