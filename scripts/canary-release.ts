...
const getRelevantChangesets = (baseBranch: string): string[] => {
  const comparePoint = spawnSync("git", [
    "merge-base",
    `origin/${baseBranch}`,
    "HEAD",
  ])
  ...
  const modifiedFiles = spawnSync("git", ["diff", "--name-only", comparePoint])
    .stdout.toString()
    .trimEnd()
    .split("\n");
...
const updateVersions = async (): Promise<void | never> => {
  ...
  const changesets = (await readChangesets(cwd)).filter((changeset) =>
    modifiedChangesets.includes(changeset.id)
  );
